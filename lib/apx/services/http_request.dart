...
class HttpRequest {
  ...
  Future<dynamic> get(String baseUri,
      {String? endpoint,
      Map<String, String>? headers,
      Map<String, String>? parameters}) async {
    debugPrint(
        'Get data from $baseUri${endpoint != null ? '/$endpoint' : ''}, parameters: ${parameters ?? {}}, headers: ${headers ?? {}}');

    try {
      final uri = Uri.parse('$baseUri${endpoint != null ? '/$endpoint' : ''}')
          .replace(queryParameters: parameters);
      final response = await http.get(uri, headers: headers);
      if (response.statusCode == 200) {
        debugPrint('Response: ${response.body}');
        final httpResponse = HttpResponse.fromJson(
  ...
  Future<dynamic> post(String baseUri,
      {String? endpoint,
