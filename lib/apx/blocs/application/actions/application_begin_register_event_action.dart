  ...
  Future<void> invoke({
    ...
    if (event.invitationCode == null || event.invitationCode!.isEmpty) {
      ...
      if (prefs.getBool('isRegistered') ?? false) {
        final domains = prefs.getStringList('domains');
        logger.i('Already registered with domains: $domains');
        stateEmitter(ApplicationReadyState(domains: domains));
      } else {
        logger.i('Not registered yet, proceeding to registration');
      ...
      if (versionCheckResponse == null || !versionCheckResponse.upgradeAble) {
        ...
        if (result.succeed && domains.isNotEmpty) {
          ...
          logger.i(
              'Registration validation failed: succeed=${result.succeed}, domains=${domains.length}');
          // 确保清除任何可能存在的旧数据
          await prefs.remove('isRegistered');
          await prefs.remove('domains');
        }

        stateEmitter(ApplicationReadyState(
            domains: domains.isNotEmpty ? domains : null));
      } else {
        logger.i('Registration failed: result is null');
        stateEmitter(const ApplicationReadyState());
      }
